...
export function setApplyI18n(fn: () => void) { applyI18nFromFlows = fn; }
...
export function confirmDialog(opts: {title: string; message: string; confirmText: string}): Promise<boolean> {
...
export function showConfirmModal(items: PickItem[]) {
    ...
    el('modalConfirm').onclick = async () => {
        ...
        try {
            const rep: import('./state').CleanReport = JSON.parse(await Clean(ids, false));
            ...
            if (del > 0 && result) {
                ...
                render.renderSummary(); render.renderToolList(); render.renderDetail(); refreshReminder(); refreshTrashInfo();
            }
            rescan();
        } catch (e) {
...
export async function refreshTrashInfo() {
    try {
        const info: import('./state').TrashInfoData = JSON.parse(await TrashInfo());
...
export async function refreshTrashList() {
    ...
    body.innerHTML = trashItems.map(it => {
        ...
        const label = isOrphan ? t('ui.orphanSection') : renderKindLabel(it.kind);
...
export function stopDownloadPoll() {
    if (downloadPoll !== null) { window.clearInterval(downloadPoll); setDownloadPoll(null); }
}
export function startDownloadPoll() {
    stopDownloadPoll();
    setLastShownPct(0);
    setDownloadPoll(window.setInterval(async () => {
        try {
            const raw = await GetDownloadProgress();
            if (!raw) return; // 已结束或未开始，由完成事件兜底
            const p = JSON.parse(raw) as {downloaded: number; total: number};
...
export function stopUninstallPoll() {
    if (uninstallPoll !== null) { window.clearInterval(uninstallPoll); setUninstallPoll(null); }
}

// 详情页「卸载」→ 起始信息（官方命令/黑名单/占用）→ 确认弹窗
export function startUninstall(toolName: string) {
    UninstallStart(toolName).then(raw => {
        const info = JSON.parse(raw) as import('./state').UninstallStartInfo;
...
export function showUninstallConfirm(info: import('./state').UninstallStartInfo) {
...
export function startUninstallPoll() {
    stopUninstallPoll();
    const t0 = Date.now();
    setUninstallPoll(window.setInterval(async () => {
        try {
            const st = JSON.parse(await GetUninstallStatus()) as import('./state').UninstallStatus;
            const out = el('unOut');
            if (out) {
...
export async function refreshReminder() {
    ...
    if (!result) { bell.classList.add('hidden'); setReminderTools([]); return; }
    ...
    setReminderTools(result.tools.filter(tool => tool.cleanableBytes > thresh));
