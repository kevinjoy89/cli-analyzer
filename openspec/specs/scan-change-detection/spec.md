...
扫描结果的每个测量顶层路径（二进制 real、dataDirs、cleanables、孤儿路径）SHALL 产生一个指纹条目，包含路径、mtime、size 与 isDir；采集 MUST 仅 stat 不递归；结果 MUST 按路径排序（序列化稳定）；不存在的路径 MUST 不产生条目。
...
`ScanIfUnchanged` SHALL：缓存与指纹均存在且指纹一致时直接返回缓存结果（不执行扫描、不写历史）；指纹文件缺失或指纹不一致时执行全量扫描；全量扫描后 MUST 同写缓存与指纹（指纹基于未过滤结果）。
...
文件内容原地修改（不改变父目录 mtime）可能不触发变更检测，SHALL 在文档中说明；强制全量路径提供兜底。
