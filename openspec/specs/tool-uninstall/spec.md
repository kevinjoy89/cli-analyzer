...
为工具提供完整的卸载能力：按安装来源给出标准卸载命令（可代跑）、卸载后检测残留数据目录、并将残留处置为可恢复或永久。残留默认移入内置回收站（可恢复）；永久删除是用户的显式选择（强确认），不再以"安全门槛"替用户裁决。
...
### Requirement: 残留处置（默认回收站，永久删除为显式选择）

系统 SHALL 将用户确认的残留项移入内置回收站（同文件系统瞬时、可恢复），作为默认处置路径。系统 SHALL 也允许用户显式选择永久删除残留（GUI 强确认 / CLI `--permanent`），该操作 SHALL 仅接受残留白名单中的路径（规则表 + 扫描快照归因），且 SHALL NOT 通过 `--yes` 单独触发——`--yes` 只跳过交互确认，不改变删除去向。

#### Scenario: 残留移入回收站
- **WHEN** 用户确认处置残留项（默认路径）
- **THEN** 残留项移入内置回收站，保留期内可恢复，状态栏回收站占用更新

#### Scenario: 永久删除残留需显式选择
- **WHEN** 用户明确选择永久删除残留（GUI 强确认或 CLI `--permanent`）
- **THEN** 残留被直接删除（不可恢复），且仅接受残留白名单内的路径

### Requirement: 残留确认交互

系统 SHALL 在残留处置前展示残留列表（默认全选），提供一次确认；含凭证的项 SHALL 标红并在确认文案中明确说明将删除配置类数据（可恢复）。CLI 的 `--yes` SHALL 跳过交互确认；`--permanent` SHALL 将删除去向改为永久删除（不可恢复），与默认的回收站去向区分。
