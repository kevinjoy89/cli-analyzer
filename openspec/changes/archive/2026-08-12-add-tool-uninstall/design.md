...
现状（详见 proposal.md - Why）：扫描器已按安装来源分类每个工具（brew/npm/pipx/cargo/go/pyenv/versioned/other），规则表持有每工具的数据目录规则（`dd(Root, Sub, Tier, Kind)`），trash 包提供可恢复的回收站移入。本设计在其上叠加"卸载"能力：标准卸载命令 + 残留双源检测 + 残留回收站清理，全程可反悔。

## Goals / Non-Goals

**Goals:**
- 用户无需知道卸载命令：工具按来源给出标准卸载命令，可代跑。
- 残留检测覆盖规则表目录与扫描归因目录（双源），卸载后一次串联完成。
- USER 级数据（配置/凭证）的唯一删除路径是内置回收站（可恢复）；不新增永久删除路径。
- CLI/GUI 双入口共享同一 core，全部文案本地化。

**Non-Goals:**
- 不做整包"强制卸载"（手动删可执行文件+全部足迹）：包管理器一致性由标准卸载保证，避免共享文件/DB 不一致风险面。
- 不做依赖图分析（无法静态得知 npm/pip 依赖关系）；以黑名单 + 标准卸载委托规避级联风险。
...
- GUI「执行标准卸载」确认后执行：`exec.Command(bin, args...)`，stdout/stderr 合并流式经事件（`uninstall:output`）推给前端；完成后推 `uninstall:done`（成功/失败+exit code）。
...
- CLI：
  - `cli-analyzer uninstall <tool>`：显示标准卸载命令 → 询问是否代跑（仅可代跑来源）→ 代跑后残留检测 → 询问是否清理残留（默认全选）→ 执行。
...
- 详情页工具头部新增「卸载」按钮（danger 样式），点击：
  1. 确认弹窗：标准卸载命令（复制按钮）+ 占用摘要 + （黑名单工具 → 直接拒绝说明）
  2. 「执行标准卸载」→ 输出区流式显示（或"复制命令自行执行"）
...
- 依赖重叠警告（其他工具共享同一包根的启发式提示）——当前由黑名单+标准卸载规避。
