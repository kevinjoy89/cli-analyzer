...
- [x] 2.3 孤儿体积惰性：仅展开时 WalkAll 计算并缓存，避免启动即全量计体积
...
- [x] 3.1 GUI 工具列表底部"未归属数据"小节：伪工具行 + 展开显示条目（路径/大小/数据根）
- [x] 3.2 新绑定：孤儿条目移入回收站（复用 trash 移入路径，USER 级、可恢复、无永久删除）
- [x] 3.3 处置后立即刷新右下角回收站占用 + 小节状态
- [x] 3.4 空态处理（无孤儿时不显示小节）+ 展开体积懒加载交互
